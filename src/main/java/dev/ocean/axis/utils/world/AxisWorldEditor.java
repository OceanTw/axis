package dev.ocean.axis.utils.world;

import dev.ocean.axis.utils.world.pattern.AxisBlockPattern;
import dev.ocean.axis.utils.world.pattern.AxisReplacePattern;
import dev.ocean.axis.utils.world.pattern.AxisSinglePattern;
import dev.ocean.axis.utils.world.task.AxisBatchProcessor;
...
public class AxisWorldEditor {
    private final Map<World, AxisBatchProcessor> processors = new ConcurrentHashMap<>();
    ...
    private static AxisWorldEditor instance;

    public static AxisWorldEditor get() {
        if (instance == null) {
            instance = new AxisWorldEditor();
        }
        return instance;
    }

    private AxisBatchProcessor getProcessor(World world) {
        return processors.computeIfAbsent(world, AxisBatchProcessor::new);
    }

    public CompletableFuture<Void> fill(Location pos1, Location pos2, BlockData blockData) {
        return fill(pos1, pos2, new AxisSinglePattern(((CraftBlockData) blockData).getState()));
    }

    public CompletableFuture<Void> fill(Location pos1, Location pos2, AxisBlockPattern pattern) {
        ...
        dev.ocean.axis.utils.world.region.AxisRegion region = new dev.ocean.axis.utils.world.region.AxisRegion(
    ...
    public CompletableFuture<Void> replace(Location pos1, Location pos2, BlockData from, BlockData to) {
        ...
        AxisBlockPattern pattern = new AxisReplacePattern(fromState, toState);
        return getProcessor(world).setBlocks(region, pattern);
    }

    public CompletableFuture<Void> save(World world) {
        AxisBatchProcessor processor = processors.get(world);
        if (processor != null) {
