...
public class BenchmarkCommand {
    ...
    @Execute(name = "")
    public void axis(@Context Player sender) {
        org.bukkit.World bukkitWorld = sender.getWorld();
        ...
        PlayerUtils.sendInfo(sender, "Starting AxisWorldEditor block placement benchmark...");
        ...
        AxisWorldEditor editor = AxisWorldEditor.get();
        editor.fill(pos1, pos2, stoneData)
                .thenCompose(unused -> {
                    long fillTime = System.nanoTime();
                    PlayerUtils.sendInfo(sender, "AxisWorldEditor Fill: " + size * size * size + " blocks in " + (fillTime - start) / 1_000_000 + "ms");
